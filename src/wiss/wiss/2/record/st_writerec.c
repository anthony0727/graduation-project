...
#
/* Module st_writerecord : write a record 
...
st_writerecord(filenum, ridptr, recaddr, reclen, trans_id, lockup, cond)
int	filenum;	/* open file number */
RID	*ridptr;	/* RID */
char	*recaddr;	/* record buffer address */
int	reclen;		/* number of bytes to write */
int     trans_id;       
short   lockup;
short	cond;

/* This routine (over)writes a record.
...
*/
{
	...
	if (checkset(&Trace2, tINTERFACE)) {
		printf("st_writerecord(filenum=%d,RID=", filenum);
		PRINTRIDPTR(ridptr);
 ...
 */
	...
	/* try to write the record in its current location */
	...
	if (e >= eNOERROR) {
		movebytes(rrptr->data, recaddr, reclen); /* write directly */
	...
	if (e < eNOERROR) { /* need a new page at the end */
		/* Do not lock the page because although r_addrec failed, the page 
		     is still locked in X mode */
	...
	if (rp != bp) { /* been moved before, change forwarding address */
		...
		(void) r_slide(bp, slotno, (int) sizeof(RID), &brptr, trans_id, 
			FALSE, cond);
