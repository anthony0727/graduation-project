...
an O2-style set.  The objects are put into a "o2-style" set:  the 
objects themselves are placed in a WiSS file and a clustered, 
unique index on the OIDS of the objects is constructed.  

Each relation has 13, four byte integer 
...
*/
    ...
    keyAttr.offset = 0; keyAttr.type = TBITS; keyAttr.length = 8;
...
*/
    ...
    {
	if ((current % 1000) == 0) printf("loading tuple %d\n",current);
...
/*
	printf("rid="); PRINTRID(rid); printf("\n");
	printf("lrid="); PRINTRID(lrid); printf("\n");
	printf ("result of bncmp = %d\n", bcmp((char*) &rid,(char *)&lrid, 8));
*/
	...
	bcopy (&rid, &key.value[0], 8);

	e = st_insertindex(index, &key, &rid, transId, TRUE, FALSE);
