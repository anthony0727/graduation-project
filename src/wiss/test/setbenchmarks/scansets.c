...
/* this program is the companion program to loaddbsets.c */

/* this program simply scans all the tuples in a set (a relation
implmented using a b-tree on the rid)  reading each
...
*/
    ...
    e = wiss_lock_file(transId, w, l_S,  COMMIT,  FALSE);
    CHECKERR ("build/wiss_lock_file", e);

    index = wiss_openindex(vol, relname, indexno, READ);
    CHECKERR("IndexScan/wiss_openindex", index);

    *((int *) boolexp[0].value) = upperlimit;

    scanid = wiss_openindexscan(w, index, &keyattr, NULL, NULL, boolexp,
	transId, TRUE, l_IS, FALSE);
    CHECKERR("IndexScan/wiss_openindexscan", scanid);
