...
begin
	...
	condition[1].operator := GT;
	assign2(condition[1].fielddesc.offset, SIDOFFSET);
	assign2(condition[1].fielddesc.length, 4);
	condition[1].fielddesc.ftype := TINTEGER;

	dummy := 30; (* this is really ugly ! *)
	movebytes(adr(condition[1].bvalue), adr(dummy), 4);
	(* the effect of the above two statements is to assign
           the integer 30 to the "value" field of the boolean term *)
