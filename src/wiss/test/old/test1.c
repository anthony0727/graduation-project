...
/* Test Program 1.
   ...
   Sequential scans are then tested on the file.
...
char	**argv;
{
	...
	*/
	...
	WISSERROR("test1/wiss_createfile", status);
	...
	*(int *)boolexp[4].value = 90;
	...
	WISSERROR("test1/wiss_destroyfile", status);
...
int	trans_id;
{
	...
	WISSERROR("test1/wiss_openfilescan", scanid);

	/* set the scan cursor to the first qualified record */
	status = am_fetchfirst(scanid, NULL,NOT);
	...
	for (count = 0; status >= eNOERROR; count++)
	{
		...
		WISSERROR("test1/wiss_readscan", status);

		/* print student record */
		PRINTST(student);

		/* set cursor to the next qualified record */
		status = am_fetchnext(scanid, NULL,NOT);
...
#define	INPUT	"student"
	...
	for (count = 0; read(unixfile, buf, SRECLENGTH); count++)
	{
		...
		WISSERROR("test1/wiss_insertrecord", status);
	}
