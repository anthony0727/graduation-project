...
#define FILENAME	"tfile12"
#define FILENAMEA       "tfile12a"
...
/* Create two files: one is empty, and the other contains one record. */
	...
	IOERROR("test12/io_init", i);
	i = bf_init();		/* initialize level 1 */
	BFERROR("test12/bf_init", i);
	i = st_init();			/* initialize level 2 */
	STERROR("test12/st_init", i);
	vol = st_mount("wiss2");  	/* mount the device wiss2 */
	STERROR("test12/st_mount", vol);
#ifdef	BUFTRACE
	BF_dumpfixed();
#endif
	...
	STERROR("test12/st_createfile", i);
	i = st_createfile(vol, FILENAMEA , 9, 90,90);
	STERROR("test12/st_createfile", i);
	ofna = st_openfile(vol, FILENAMEA, WRITE);
	STERROR("test12/st_openfile", ofna);
	i = st_insertrecord(ofna, recadr, len, NULL, &rida);
	STERROR("test12/st_insertrecord", i);
	i = st_closefile(ofna);
	STERROR("test12/st_closefile", i);
#ifdef	BUFTRACE
	BF_dumpfixed();
#endif
...
/* 1. update a read-only file */
	...
	STERROR("test12/st_openfile", ofn);

	i = st_insertrecord(ofn, recadr, len, NULL, &rid);
	TERROR("test12/st_insertrecord", i);
	i = st_appendrecord(ofn, recadr, len, &rid);
	TERROR("test12/st_appendrecord", i);
	i = st_deleterecord(ofn, &rid);
	TERROR("test12/st_deleterecord", i);
	i = st_writerecord(ofn, &rid, recadr, len);
	TERROR("test12/st_writerecord", i);
	i = st_closefile(ofn);
	STERROR("test12/st_closefile", i);
#ifdef	BUFTRACE
	BF_dumpfixed();
...
/* 2. testing error detection in insert, delete, read, write */
	...
	STERROR("test12/st_insertrecord", i);
	i = st_deleterecord(ofn, &rid);
	STERROR("test12/st_deleterecord", i);
...
/* 3. testing firstfile, lastfile, ... */
	...
	STERROR("test12/st_lastfile", i);
	i = st_nextfile(ofna, &rid1, &rid2);
	TERROR("test12/st_nextfile", i);
#ifdef	BUFTRACE
	BF_dumpfixed();
#endif
