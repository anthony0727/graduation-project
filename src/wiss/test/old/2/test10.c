...
#define DUMPFILE	ofn = st_openfile(vol, FILENAME, READ);\
			STERROR("test10/st_openfile", ofn);\
			dumpfile_forward(ofn);\
			i = st_closefile(ofn);\
			STERROR("test10/st_closefile", i)
...
int	ofn;
{
	...
	for(; i >= 0; rid1 = rid2) {	
		...
		STERROR("test10/st_readrecord", i);
...
/* 1. insert 10 short records */
	...
	STERROR("test10/st_openfile(for insertion)", ofn);
	for (i = 0; i < 10; i++) {
		j = st_insertrecord(ofn, rec[i], SHORT, NULL, &rid[i]);
		STERROR("test10/st_insertrecord", j);
	}
...
/* 2. delete 5 short records */
	...
	STERROR("test10/st_mount", vol);
	ofn = st_openfile(vol, FILENAME, WRITE);
	STERROR("test10/st_openfile(for deletion)", ofn);
	for (i = 0; i < 10; i = i + 2) {
		j = st_deleterecord(ofn, &rid[i]);
		STERROR("test10/st_deleterecord", j);
	}
	printf(" after deletions, the order should be 1,3,5,7,9\n");
	i = st_closefile(ofn);
	STERROR("test10/st_closefile(for deletion)", i);
	DUMPFILE;
	i = st_dismount("wiss2");  /* dismount the device wiss2 */
	STERROR("test10/st_dismount", i);
...
/* 3. append 5 short records */
	...
	STERROR("test10/st_mount", vol);
	ofn = st_openfile(vol, FILENAME, WRITE);
	STERROR("test10/st_openfile(for append)", ofn);
	for (i = 0; i < 10; i = i + 2) {
		j = st_appendrecord(ofn, rec[i], SHORT, &rid[i]);
		STERROR("test10/st_appendrecord", j);
	}
	printf(" the order should be 1,3,5,7,9,0,2,4,6,8\n");
	i = st_closefile(ofn);
	STERROR("test10/st_closefile(for append)", i);
	DUMPFILE;
	i = st_dismount("wiss2");  /* dismount the device wiss2 */
	STERROR("test10/st_dismount", i);
...
/* 4. expand(write) 5 short records to long records */
	...
	STERROR("test10/st_mount", vol);
	ofn = st_openfile(vol, FILENAME, WRITE);
	STERROR("test10/st_openfile(for write)", ofn);
	for (i = 1; i <=  9; i = i + 2) {
		sprintf(&rec[i][12], "long");
		rec[i][16] = ' ';
		j = st_writerecord(ofn, &rid[i], rec[i], LONG);
		STERROR("test10/st_writerecord", j);
	}
	printf(" 1,3,5,7,9 should be long records\n");
	i = st_closefile(ofn);
	STERROR("test10/st_closefile(for write)", i);
	DUMPFILE;
	i = st_dismount("wiss2");  /* dismount the device wiss2 */
	STERROR("test10/st_dismount", i);
...
/* finalization */
	...
	STERROR("test10/st_destroyfile", i);
	i = st_dismount("wiss2");  /* dismount the device wiss2 */
	STERROR("test10/st_dismount", i);
