...
/* step 7. delete all indices and drop the index files */
	...
	STERROR("test5/st_dropbtree", errorcode);
	errorcode = st_dropbtree(vol, TESTFILE, 3);
	STERROR("test5/st_dropbtree", errorcode);

/* clear up */
	errorcode = st_destroyfile(vol, TESTFILE);
	STERROR("test5/st_destroyfile", errorcode);
	errorcode = st_dismount("wiss2");  
	STERROR("test5/st_dismount", errorcode);
...
char	*filename;	/* where the records are to be retrieved */
{
	...
	STERROR("test5/st_openfile", openfile_number);
	...
	for ( errorcode = st_firstfile(openfile_number, &currid); 
		...
		STERROR("test5/st_readrecord", errorcode);
	...
	STERROR("test5/st_closefile", errorcode);
}
...
#define	INPUT	"studentdata/student"
	...
	while (read(unixfile, buf, SRECLENGTH)) {
		...
		for (i = 0; i < 20; i++) student.name[i] = buf[3 + i]; 		
		...
		STERROR("test5/st_insertrecord", errorcode);
		j++;
	}
...
KEY	*key;
{
	...
	e = st_getindex(indexofn, key, &cursor, &rid);
