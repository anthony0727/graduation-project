...
/* step 1. create an empty data file */
	...
	STERROR("test7/st_createfile", errorcode);
...
/* step 2. create an (empty) index file */
	...
	errorcode = st_createindex(vol, TESTFILE, 2, &keyattr[1], 
			50, TRUE, FALSE);
	STERROR("test7/st_createindex", errorcode);
#ifdef	BUFTRACE
...
/* step 4. create an index file by insertion */
...
/* step 4.a index scan */
	...
	STERROR("test7/st_indexscan", errorcode);
#ifdef	BUFTRACE
	BF_dumpfixed();	/* check if any buffer fixed in the buffer pool */
#endif

/* step 5. delete all indices and drop the index file */

	printf(" deleting all the indices from the index file\n");
	...
	STERROR("test7/st_dropbtree", errorcode);

/* step 6. create the index file directly (from bottom up) */

	printf(" creating the index file from bottom up\n");

	errorcode = st_createindex(vol, TESTFILE, 2, 
			&keyattr[1], 100, TRUE, FALSE);
	STERROR("test7/st_createindex", errorcode);
#ifdef	BUFTRACE
	BF_dumpfixed();	/* check if any buffer fixed in the buffer pool */
#endif

	i = st_openbtree(vol, TESTFILE, 2, READ);
	STERROR("test7/st_openbtree", i);

	errorcode = st_closefile(i);
	STERROR("test7/st_closefile", errorcode);
...
/* clear up */
	...
	STERROR("test7/st_destroyfile", errorcode);

	/* dismount the volume call "wiss2" */
	errorcode = st_dismount("wiss2");  
	STERROR("test7/st_dismount", errorcode);
#ifdef	BUFTRACE
	BF_dumpfixed();	/* check if any buffer fixed in the buffer pool */
#endif
...
char	*filename;	/* where the records are to be retrieved */
{
	...
	STERROR("test7/st_openfile", openfile_number);
					...
					errorcode >= eNOERROR;)
	{
		...
		STERROR("test7/st_readrecord", errorcode);
...
#define	INPUT	"studentdata/student"
	...
	while (read(unixfile, buf, SRECLENGTH))
	{
		...
		STERROR("test7/st_insertrecord", errorcode);
	}
...
int	index;		/* index number */
{
	...
	STERROR("test7/st_openbtree", indexfile_number);
					...
					errorcode >= eNOERROR;)
	{
		...
		STERROR("test7/st_insertindex", errorcode);
	...
	pid = F_ROOTPID(indexfile_number);
	fid = F_FILEID(indexfile_number);

/*
	printf(" after inserting all indices into index file %d\n", index);
	bt_print_btfile(indexfile_number, &pid);
*/
...
int	index;		/* index # of the index file */
{
					...
					errorcode >= eNOERROR;)
	{
		...
		STERROR("test7/st_deleteindex", errorcode);
...
/*
	printf(" after deleting all indices from index %d\n", index);
	bt_print_btfile(indexfile_number, &pid);
*/
