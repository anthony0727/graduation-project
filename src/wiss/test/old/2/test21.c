...
/* initialization */
	...
	IOERROR("test21/io_init", e);
	e = bf_init();		/* initialize level 1 */
	BFERROR("test21/bf_init", e);
	e = st_init();			/* initialize level 2 */
	STERROR("test21/st_init", e);
	vol = st_mount("wiss2");  	/* mount the device wiss2 */
	STERROR("test21/st_mount", vol);
	e = st_createfile(vol, FILENAME, 9, PFF,PFF);
	STERROR("test21/st_createfile", e);
	ofn = st_openfile(vol, FILENAME, WRITE);
	STERROR("test21/st_openfile(for insertion)", ofn);
	e = st_createlong(ofn, &lrid);
	STERROR("test21/st_createlong", e);
#ifdef	BUFTRACE
	BF_dumpfixed();
#endif
...
/* 1. insert (append) 10 cells */
	...
	for (i = 0; i < 10; i++) {	
		...
		STERROR("test21/st_insertframe", e);
		r_dumplong(ofn, &lrid);
	}
#ifdef	BUFTRACE
	BF_dumpfixed();
...
/* 4. write frame */
	...
	for (k++, j = 8; j >= 1; j /= 2 , k++) {
		...
		STERROR("test21/st_writeframe", e);
		r_dumplong(ofn, &lrid);
	}
#ifdef	BUFTRACE
	BF_dumpfixed();
...
/* 6. delete cells from the middle */
	...
	for (j = 8; j >= 1; j /= 2 ) {
		...
		STERROR("test21/st_deleteframe", e);
		r_dumplong(ofn, &lrid);
	}
#ifdef	BUFTRACE
	BF_dumpfixed();
...
/* 7. compress */
	...
	STERROR("test21/st_compresslong", e);
	r_dumplong(ofn, &lrid);
#ifdef	BUFTRACE
	BF_dumpfixed();
#endif
...
/* 8. destory long */
	...
	STERROR("test21/st_destroylong", e);
	e = st_closefile(ofn);
	STERROR("test21/st_closefile(for destroy_long", e);
#ifdef	BUFTRACE
	BF_dumpfixed();
#endif
...
/* finalization */
	...
	STERROR("test21/st_destroyfile", e);
	e = st_dismount("wiss2");  /* dismount the device wiss2 */
	STERROR("test21/st_dismount", e);
