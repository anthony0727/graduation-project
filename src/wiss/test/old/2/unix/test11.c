...
/* 3. negative record length */
	...
	ZERROR("test11/st_insertrecord", i);
	i = st_appendrecord(ofn, recadr, -1, &rid);
	ZERROR("test11/st_appendrecord", i);
	i = st_readrecord(ofn, &rid, recadr, -1);
	ZERROR("test11/st_readrecord", i);
	i = st_writerecord(ofn, &rid, recadr, -1);
	ZERROR("test11/st_writerecord", i);
#ifdef	BUFTRACE
	BF_dumpfixed();
#endif
...
/* finalization */
	...
	i = st_closefile(ofn);
	STERROR("test11/st_closefile", i);
	i = st_destroyfile(vol, FILENAME);
	STERROR("test11/st_destroyfile", i);
	i = st_dismount("wiss2");  /* dismount the device wiss2 */
	STERROR("test11/st_dismount", i);
