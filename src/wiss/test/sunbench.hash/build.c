...
char    **argv;
{
    ...
    /* 50 for 8K byte pages,  25 for 4K byte pages */
    e = wiss_createfile(vol, FileName, (partCnt/50), 100,90);
    CHECKERR("build/wiss_createfile", e);

    e = wiss_createhash(vol, FileName, HNO, &keyattr, 90, TRUE, 
			    trans_id, lockup, cond);
    CHECKERR("build/wiss_createhash", e);
...
*/
	...
	e = wiss_inserthash(hofn, &key, &curPartRid, trans_id, lockup, cond);
...
*/
		...
		bcopy(&next, key.value, sizeof(next));
	        e = wiss_gethash(hofn, &key, &cursor, &toPartRid, trans_id, 
			lockup, cond);
	        CHECKERR("build/wiss_gethash", e);
...
expansionCnt++;
    ...
    printf("Size of hash index = %d pages\n",
	wiss_hashpages(vol,FileName,HNO));
