...
char    **argv;
{
	...
	newPartNo = dbSize+1;  /* starting part number of new parts */
	    ...
	    e = wiss_insertindex(hofn, &key, &curPartRid, trans_id, 
		lockup, cond);
	    CHECKERR("build/wiss_insertindex", e);
        }
	    ...
	    */
	    ...
	    for (j=0;j<3;j++)
	    {
		    ...
		    bcopy (&next, key.value, sizeof(next));
	            e = wiss_getindex(hofn, &key, &cursor, &toPartRid, 
			trans_id, lockup, BT_READ, cond);
	            CHECKERR("build/wiss_getindex", e);
...
int	firstPartId, partCnt, ofn, hofn, trans_id, lockup, cond;
{
	...
	e = wiss_getindex(hofn, &key, &cursor, &partRid, trans_id, lockup, 
		BT_READ, cond);
	...
	e = wiss_getindex(hofn, &key, &cursor, &partRid, trans_id, lockup,
        ...
        /* now remove the part from the index */
	e = wiss_deleteindex(hofn, &key, &partRid, trans_id, lockup, cond);
