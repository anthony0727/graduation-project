...
r_reservepage(filenum, pidptr, pageptr, trans_id, lockup, mode, cond)
...
short           cond;
{
	...
	e = bf_reservepage(trans_id, filenum, FC_FILEID(filenum), &pid, &dp);
	CHECKERROR(e);
...
r_reserve_lock_page(filenum, pidptr, pageptr, trans_id, lockup, mode, cond)
int		filenum;	/* open file number */
PID		*pidptr;	/* *RID */
DATAPAGE	**pageptr;	/* pointer to page buffer */
int             trans_id;
short           lockup;
LOCKTYPE	mode;
short           cond;
{
	...
	e = bf_reserve_lock_page(trans_id, filenum, FC_FILEID(filenum), &pid, &dp);
	CHECKERROR(e);
