...
/* repeatly creating files and then destorying them, 
   ...
   NOTE : The difference between this test and test14 is that test14 mounts
   	  the volume each time a file is created.
*/
...
/* testing st_createfile */
	...
	for (j = 0; j < 25; j++)
	{
		...
		STERROR("test14/st_createfile", i);
	}

	i = st_dismount("wiss2");  /* dismount the device wiss2 */
	STERROR("test14/st_dismount", i);
...
/* testing st_destroyfile */
	...
	for (j = 0; j < 25; j++)
	{
		...
		STERROR("test14/st_destroyfile", i);
	}
