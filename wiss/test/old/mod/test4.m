...
(* error handler *)
#define	checkerror(p,c)	if (c < 0) then am_fatalerror(adr(p), c); halt;end
...
const
	...
	SRECLENGTH	= 34;	/* length of a record in the UNIX file */
...
begin
	...
	checkerror("test4/wiss_openfile", openfile_number);
	...
	while (read(unixfile, buf, SRECLENGTH) > 0) do
		...
		checkerror("test4/wiss_insertrecord", errorcode);

	end; (* while *)

	(* close the wiss file *)
	errorcode := wiss_closefile(openfile_number);
	checkerror("test4/wiss_closefile", errorcode);
...
begin
	...
	checkerror("test4/wiss_openindex", indexfile_number);
	...
	checkerror("test4/wiss_openfilescan", scanid);
	...
	while (errorcode >= eNOERROR) do
		...
		checkerror("test4/wiss_readscan", errorcode);
	...
	checkerror("test4/wiss_closescan", errorcode);
...
begin
	...
	checkerror("test4/wiss_mount", vol);
	...
	checkerror("test4/wiss_createfile", errorcode);
...
*)
	...
	checkerror("test4/wiss_createindex", errorcode);
	...
	checkerror("test4/wiss_destroyfile", errorcode);
	errorcode := wiss_dropindex(vol, FILENAME, AGEINDEX);
	checkerror("test4/wiss_dropindex", errorcode);

	(* dismount the volume call DEVICENAME *)
	errorcode := wiss_dismount(DEVICENAME);  
	checkerror("test4/wiss_dismount", errorcode);
