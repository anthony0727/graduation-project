...
*/
    ...
    e = st_firstscan (ofnrids, &rootrid, &recptr, &reclen, 
			transId, TRUE, l_S, FALSE); 
...
*/
	...
	/* recptr points to the data portion of the next record */
	bcopy (recptr, &rid, reclen);
...
*/
	...
	cnt++;
    	e = st_scanfile (ofnrids, &rootrid, &rootrid, &recptr, &reclen,
		transId, TRUE, l_S, FALSE);
    }
    return(cnt);
} /* end bf_scan() */
